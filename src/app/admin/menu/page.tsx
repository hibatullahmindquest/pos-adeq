      ...
      <div className="flex-1 overflow-y-auto p-6 lg:p-8 max-w-6xl w-full mx-auto">
        <div className="flex items-center justify-between mb-5 flex-wrap gap-3">
          <div className="flex items-center gap-3 flex-wrap">
            <div className="text-lg font-extrabold text-ink">Menu</div>
            <input
              value={search}
              onChange={(e) => setSearch(e.target.value)}
              placeholder="Cari item…"
              className="border border-border bg-white rounded-lg px-3.5 py-2.5 text-[12.5px] text-ink placeholder:text-faint w-56"
            />
          </div>
          <Button variant="primary" className="!min-h-0 !py-2.5" onClick={openAdd}>
            + Tambah Item Baru
          </Button>
        </div>
        ...
        <div className="bg-white border border-border rounded-2xl overflow-hidden">
          <div className="grid grid-cols-[2fr_1fr_1fr_1fr_110px] px-5 py-3 bg-muted-bg text-[11.5px] font-extrabold text-muted uppercase tracking-wide">
            <span>Nama Item</span>
            <span>Kategori</span>
            <span>Harga</span>
            <span>Status</span>
          ...
          {filtered.map((item) => (
            <div
              key={item.id}
              className={`grid grid-cols-[2fr_1fr_1fr_1fr_110px] px-5 py-3.5 items-center border-t border-border-light ${
